...
class EnlightenmentCenter extends Robot {
    ...
    Map.Entry<MapLocation, Integer> minNeutral;
    Map.Entry<MapLocation, Integer> minEnemy;
    ...
    private RobotType getUnitToBuild() throws GameActionException {
        ...
        if (rc.getRoundNum() <= 2) {
        ...
        } else if ((rand > 0.6 || crowded(rc.getLocation())) && ((!neutralHQs.isEmpty()
            ...
            return RobotType.POLITICIAN;
        } else if (rand > (0.1 + 0.2 * rc.getRoundNum() / Constants.MAX_ROUNDS) || maxInf / 2 < Constants.minimumPolInf
                || canSenseEnemyPolitician()) {
            return RobotType.MUCKRAKER;
        } else if (rand < (0.1 - 0.1 * rc.getRoundNum() / Constants.MAX_ROUNDS) && !canSenseEnemyMuckraker()
