...
class EnlightenmentCenter extends Robot {
	...
	/**
	 * Builds a unit when possible and getUnitToBuild() returns a non-default value,
	 * and bids when optimal and possible to win more votes than the opponent.
	 * In addition, communicates to spawned units by encoding tasks in its flag.
	 */
			...
			if (rc.isReady() && !fullySurrounded()) {
				...
				if (unitToBuild != RobotType.ENLIGHTENMENT_CENTER) {
					...
					if (!unitToBuild.equals(RobotType.SLANDERER) && Constants.optimalSlandInfSet.contains(infToSpend)) {
						// Allows politicians to distinguish slanderers by their influence, even though
						// they don't have "true sense"
						infToSpend--;
					}
	...
	MapLocation slandCenter() {
		...
		MapLocation center = rc.getLocation().translate(slandDistAway * awayFromEnemyHQs.dx,
	...
	private RobotType getUnitToBuild() throws GameActionException {
		...
		if (rc.getRoundNum() <= 2) {
			...
			return RobotType.POLITICIAN;
				...
				&& ((minNeutral != null && maxInf * rc.getEmpowerFactor(allyTeam, 20) >= minNeutral.getValue())
	...
	private class Bidding {
		...
		private double getBidMultiplier() {
			...
			if (rc.getTeamVotes() < lowerVote || rc.getTeamVotes() > upperVote) {
				/*
				 * Even if we cannot secure 751 votes, we can still potentially win more votes
				 * than the opponent since it is possible for neither player to win a vote in a
				 * given round.
				 */
				return 1;
			}
